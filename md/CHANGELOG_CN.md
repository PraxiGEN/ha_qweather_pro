...
### 📣 用户须知（配置与重新配置）
- **默认使用 JWT 认证：** 新接入与重新配置默认采用 JWT（Ed25519）鉴权；API KEY 方式已强制限制请求频率（锁定刷新间隔），且集成会通过"修复"中的 Repairs 条目提醒仍在使用 API KEY 的用户迁移至 JWT。
- **JWT 新增开发者 ID（项目ID / project_id）输入项：** 因和风天气现阶段审核不严格，该字段暂不强制校验；但为避免后期校验收紧导致错误，请在"重新配置"中补全开发者 ID 字段（与密钥ID key_id、签发者 iss 一并核对）。
- **天气实体属性精简，新增数据查询服务：** 天气实体属性已做精简；完整数据请改用新增的 `qweather_pro.get_weather` 服务获取，该服务可返回全部天气数据（实况、逐时/每日预报、空气质量、预警、生活指数等），并支持按 `keys` 过滤。
